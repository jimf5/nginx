    ...
    ngx_http_core_loc_conf_t *clcf, ngx_uint_t err)
{
    ...
    if (ngx_http_error_pages[err].len) {
        ...
        if (err == (NGX_HTTP_MOVED_PERMANENTLY - 301 + NGX_HTTP_OFF_3XX)
            || err == (NGX_HTTP_MOVED_TEMPORARILY - 301 + NGX_HTTP_OFF_3XX)
            || err == (NGX_HTTP_TEMPORARY_REDIRECT - 301 + NGX_HTTP_OFF_3XX)
            || err == (NGX_HTTP_PERMANENT_REDIRECT - 301 + NGX_HTTP_OFF_3XX))
        {
